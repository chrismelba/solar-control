...
class Battery(Device):
    ...
    def to_dict(self) -> dict:
        ...
        battery_dict = {
            ...
            'battery_percentage_sensor': self.battery_percentage_sensor,
            'max_charging_speed': self.max_charging_speed,
            'force_charge_switch': self.force_charge_switch
        }
        base_dict.update(battery_dict)
        return base_dict

    @classmethod
    def from_dict(cls, data: dict) -> 'Battery':
        # Extract battery-specific fields
        battery_data = {
            'battery_size': data.pop('battery_size'),
            'battery_percentage_sensor': data.pop('battery_percentage_sensor'),
            'max_charging_speed': data.pop('max_charging_speed', None),
            'force_charge_switch': data.pop('force_charge_switch', None)
        }
        
        # Create base device
        device = super().from_dict(data)
        
        # Create battery with base device data
        return cls(
            **device.__dict__,
            **battery_data
        )
    ...
    def get_battery_percentage(self) -> Optional[float]:
        """Get the current battery percentage from Home Assistant"""
        if not self.battery_percentage_sensor:
            return None
        ...
        try:
            ...
            response = requests.get(
                f"{hass_url}/api/states/{self.battery_percentage_sensor}",
                headers=headers
            )
            response.raise_for_status()
    ...
    def is_force_charging(self) -> bool:
        """Check if force charging is enabled"""
        if not self.force_charge_switch:
            return False
        ...
        try:
            ...
            response = requests.get(
                f"{hass_url}/api/states/{self.force_charge_switch}",
                headers=headers
            )
            response.raise_for_status()
            ...
            logger.error(f"Failed to get force charge state: {e}")
            return False 